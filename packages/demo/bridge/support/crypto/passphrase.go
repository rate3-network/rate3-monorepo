...
func PromptPassphrase(requireConfirmation bool) (passphrase []byte, err error) {
	var isValidPassword bool

	for !isValidPassword {
		passphrase, err = terminal.PromptPassword("Enter passphrase: ")
		if err != nil {
			return nil, err
		}
		if len(passphrase) == 0 {
		...
		if requireConfirmation {
			confirmation, err := terminal.PromptPassword("Re-enter passphrase: ")
			if err != nil {
				return nil, err
			}
